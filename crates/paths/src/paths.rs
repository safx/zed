...
/// The capitalized application name (e.g., "Zed", "Agentium"), used for
/// macOS ~/Library/* paths and Windows paths.
static APP_NAME: OnceLock<String> = OnceLock::new();

/// The lowercase application name (e.g., "zed", "agentium"), used for
/// Unix config/data directories.
static APP_NAME_LOWER: OnceLock<String> = OnceLock::new();

/// Sets the application name used in all path computations.
...
fn app_name() -> &'static str {
    APP_NAME.get().map(|s| s.as_str()).unwrap_or("Zed")
}

fn app_name_lower() -> &'static str {
    APP_NAME_LOWER.get().map(|s| s.as_str()).unwrap_or("zed")
}
